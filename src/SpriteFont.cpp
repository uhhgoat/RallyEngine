...
SpriteFont::SpriteFont(std::string name, std::string path)
{
	...
	XMLtoCollection((DefaultPath + path + ".xml").c_str());
}

SpriteFont::~SpriteFont()
{
	// to be completed
	for (std::map<Key, Glyph*>::iterator it = fontmap.begin(); it != fontmap.end(); it++)
	{
		delete it->second;
	}
}
	...
	while (S_OK == reader->Read(&nodeType))
	{
		...
		case XmlNodeType::XmlNodeType_EndElement:
		{
			...
			if (wcscmp(stringValue, L"character") == 0)
			{
				...
				ImageManager::Load(Name + std::to_string(key), FontTexture, new Rect(float(x), float(y), float(w), float(h)));

				Glyph* glyph = new Glyph();
				glyph->SetImage(ImageManager::Get(Name + std::to_string(key)));
				glyph->SetCenter(0, 0);
